import React from 'react';
import PropTypes from 'prop-types';
import StyledInfo from './styledInfo';

export default function InfoIcon({ disabled }) {
  return (
    <StyledInfo
      disabled={disabled}
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M10 18.3334C14.6024 18.3334 18.3333 14.6025 18.3333 10.0001C18.3333 5.39771 14.6024 1.66675 10 1.66675C5.39763 1.66675 1.66667 5.39771 1.66667 10.0001C1.66667 14.6025 5.39763 18.3334 10 18.3334Z"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path d="M10 13.3333V10" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M10 6.66675H10.0083" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </StyledInfo>
  );
}

InfoIcon.propTypes = {
  disabled: PropTypes.bool,
};

InfoIcon.defaultProps = {
  disabled: false,
};
