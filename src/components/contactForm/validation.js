...
const validateName = (name) => {
  ...
  if (name.length > MAX_NAME_LENGTH)
    return { isValid: false, isInvalid: true, message: 'Zbyt długi imię.' };
...
const validateLastName = (lastName) => {
  ...
  if (lastName.length > MAX_NAME_LENGTH)
    return { isValid: false, isInvalid: true, message: 'Zbyt długi nazwisko.' };

  return { isValid: true, isInvalid: false, message: 'ok' };
};
const validateEmail = (email) => {
  if (!email) return { isValid: false, isInvalid: true, message: 'Niepoprawny adres email.' };
